...
[nPsFast,nPsSlow,ntg] = nPhotons(XpsFast,XpsSlow,Xtg);
At=sqrt(2)*r*sqrt((nPsFast+nPsSlow)*ntg)/(1+nPsFast+nPsSlow);
...
% 
        ...
        nDs3(i) = n3sel;
        nOtg = mean(nDs3);
        Aps(i)= At*(1+(n1sel+n2sel))/sqrt(2*((n1sel+n2sel)*nOtg));
...
%savefig('15,100mm_withAps_test.fig');